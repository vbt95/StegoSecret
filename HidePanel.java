...
/** Constructor to set up hide panel
...
/** Action listener for various buttons
...
public void actionPerformed(ActionEvent ae)
{
    ...
    {
        // if file is selected successfully
        if(browseValue==JFileChooser.APPROVE_OPTION)
        {
            if(getPass().equals(getRepass()))
            {
                // password length is limited to 8 characters to avoid overhead in size of image
                if(getPass().length()<=8)
                {
...
/** To reset all fields of hide panel
 * 
 */
public void reset()
{
...
/** Returns the password entered in re-password field
 */
